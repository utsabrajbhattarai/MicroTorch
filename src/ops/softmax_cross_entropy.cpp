...
TensorPtr softmax_cross_entropy(const TensorPtr& logits, const Eigen::MatrixXd& targets){
    ...
    //cross_entropy:
    double loss = -(targets.array() * probs.array().log()).sum() / probs.rows();
    ...
    out->backward_fnc = [logitsp, outp, targets, probs]() {  
        double scale = outp->grad(0,0) / probs.rows();
        logitsp->grad += scale * (probs - targets);
    };
